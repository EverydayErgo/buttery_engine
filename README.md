...
### Useful functions
You can check current pseudolayer with:
```C
    bool is_pseudolayer_on(uint8_t pseudolayer);
```
Pseudolayers are generated by Python parser, you can find them in **introspection.h**
```C
    enum pseudolayers {
    ALWAYS_ON, Base, Num_123, FN_keys, Arrows, PDScroll_V, PDScroll_H, Modkeys_L, Modkeys_R, Numpad, Specials
};
````
